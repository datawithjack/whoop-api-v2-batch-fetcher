...
def find_user_csv_file(user_email):
    """Find the most recent CSV file for a specific user"""
    exports_dir = "exports"
    pattern = os.path.join(exports_dir, f"sleep_data_batch_{user_email.replace('@', '_at_').replace('.', '_')}_*.csv")
...
def update_user_sleep_data(user_email, user_credentials):
    ...
    if check_token_validity(user_credentials):
        ...
        if refresh_user_token_batch(user_credentials):
            ...
            if test_user_token(user_credentials):
                ...
                print(f"     python src/whoopy_auth_batch.py")
    ...
    if not csv_file:
        print(f"  ❌ No existing CSV file found for {user_email}")
        print("  💡 Run batch_sleep_fetcher.py first to create initial data file")
        return False
...
def main():
    ...
    print("🔄 WHOOP Sleep Data Updater")
    print("=" * 50)
